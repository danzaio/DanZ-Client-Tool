...
I'm happy to be releasing the DanZ Client Tool! It revitalizes many features from the original KBot concept. This significant update, while ensuring constant updates and a new UI, it was primarily focused on **enhancing its operational discretion**. It’s free, remains in active development, and more stuff is soon to come :)
...
While the source is not publicly available by default, I am committed to transparency for those with genuine interest or concerns. If you have any doubts about how a specific feature works, or if you're a developer interested in understanding parts of the codebase, please feel free to DM me – I'll happily share relevant source snippets or explanations. Furthermore, if there's strong community interest and consensus, I am open to fully open-sourcing the project in the future.
...
![image](https://github.com/user-attachments/assets/a157723b-f758-4d96-a9cd-a7e3b1a2d5e1)

![image](https://github.com/user-attachments/assets/ff27898d-3a3b-4fea-9ff8-7653f729c5da)
![image](https://github.com/user-attachments/assets/9fbdd56e-28b5-4295-ad33-f3b0c2c73b6c)
![image](https://github.com/user-attachments/assets/ecf491e8-ab9d-4927-9169-80ab55a3134b)
![image](https://github.com/user-attachments/assets/1d7801c0-daa6-41fc-8e67-2e420224d1e8)
---

## ⚙️ Prerequisites

*   Windows Operating System (due to `psutil` process searching and `win32api` for file version, though `win32api` can be made optional).
...
2.  **Run:**
    *   Ensure your League of Legends client is running and you are logged in.
    *   Run the downloaded executable.
3.  **Connect:**
    *   The tool will attempt to automatically connect to your League client. The connection status is shown in the bottom status bar.
    *   If not connected, try the "Reconnect" button.

---

## 📖 Detailed Functionality

The tool is organized into several tabs, each catering to different functionalities:
...
*   **Results Textbox:** Displays the JSON response from the LCU for the lookup.
...
*   **Background:**
    ...
    *   **Skin ComboBox:** Select a skin for the chosen champion. (Populates based on champion selection).
    *   **Champion Icon & Skin Splash Preview:** Shows a preview of the selected champion's icon and skin's splash art.
    *   **Set Background Button:** Applies the selected champion skin as your profile background.
*   **Chat Connection:**
    *   **Disconnect Chat Button:** Disconnects your client from Riot's chat servers (appears offline to others but you can still see messages).
...
*   **Automatic Actions:**
    *   **Auto Accept Queue Switch:** Automatically accepts queue when it pops.
    *   **Instalock Switch:** Enables instalocking.
        *   **Champion ComboBox:** Select the champion to instalock (lists owned/F2P champs).
        *   **Delay Entry (ms):** Set delay before locking in.
        *   **Backup Champion ComboBox:** Select a backup champion if your primary instalock choice is banned/picked.
        *   **Dodge if Instalock Banned Switch:** If your primary instalock champ is banned/picked by the enemy, automatically dodge the queue (only works if Instalock is enabled).
    *   **Autoban Switch:** Enables auto-banning.
        *   **Champion ComboBox:** Select the champion to auto-ban (lists all champs).
        *   **Delay Entry (ms):** Set delay before banning.
...
*   **Instant Message (Champ Select):**
    *   **Message Entry:** Text to be sent in champ select.
    *   **Delay Slider (ms):** Initial delay before sending the first message.
    *   **Times Entry:** How many times to send the message.
    *   **Delay Between Slider (ms):** Delay between sending multiple messages.
...
*   **Skins Table:** Displays a sortable list of your owned skins with columns for Skin Name, Champion, Skin ID, Purchase Date, and Purchase Time.
...
*   **Remove Friends from Group:**
    *   **Friend Group ComboBox:** Select a friend group (including "Default (All Friends)"). Populates after LCU connection.
    *   **Remove Friends From: Button:** PERMANENTLY removes all friends belonging to the selected group. **Use with extreme caution!**
*   **Change Riot ID:**
    *   **Game Name Entry:** Desired new game name.
    *   **#Tag Entry:** Desired new tag line (without the '#').
...
*   **Auto Accept Worker:** Periodically checks if a queue is ready and, if "Auto Accept Queue" is enabled, accepts it.
*   **Champ Select Actions Worker:**
    *   Monitors the champ select phase.
    *   If **Instalock** is enabled: Picks the selected champion (or backup, or random) after the configured delay. Handles dodging if the primary instalock champ is banned/picked and "Dodge on Ban" is enabled.
    *   If **Autoban** is enabled: Bans the selected champion after the configured delay.
    *   If **Instant Message** text is set: Sends the configured message(s) once per champ select phase.
    *   If **Instant Mute Team** is enabled: Mutes teammates once per champ select phase.
    *   If **Side Notification** is enabled: Sends a chat message indicating your team's side (Blue/Red) once per champ select phase.
*   **Riot Client Version Updater:** Periodically fetches the latest Riot Client version string, which can be useful for some LCU authentication headers (though current LCUHandler is robust).
